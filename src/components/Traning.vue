  ...
  <div class="mx-3">
    <div class="header sticky">
      ...
      <div @click="resetSelect()" style="grid-area: text1">
        {{
          selectedCard
            ? `selected card: ${findCard(selectedCard)?.name}(${selectedFrom})`
            : "no card selected"
        }}
      </div>
      ...
      <div
        ...
        v-if="selectedCard"
        class="orange text-dark round text-center"
        @click="targetCard()"
      >
        target
      </div>
      <div
        style="grid-area: effect"
        v-if="selectedCard"
        class="orange text-dark round text-center"
        @click="activeEffect()"
      >
    ...
    <div v-if="view == 'special'">
      <div>Special</div>
      <div class="special">
        <div class="orange text-dark me-1" @click="specialAction('pendel')">
          pendel
        </div>
        <div class="orange text-dark me-1" @click="specialAction('token')">
          token
        </div>
        <div class="orange text-dark me-1" @click="specialAction('addCard')">
          add card
        </div>
        <div class="orange text-dark me-1" @click="specialAction('hide')">
          {{ hide ? "show" : "hide" }}
        </div>
        <div
          ...
          v-if="
            selectedFrom.includes('monster') ||
            selectedFrom.includes('spell') ||
            selectedFrom.includes('extra')
          "
        >
          ...
          <div class="w-100">
            <div class="mb-1">
              <div>
                <Button
                  @click="reset()"
                  type="button"
                  class="btn orange w-100 round mt-1"
                >
                  <template v-slot:button> Reset</template>
                </Button>
              </div>
            </div>
            <div class="mb-3">
              <div>
                <Button
                  @click="changeStartHand()"
                  type="button"
                  class="btn orange w-100 round mt-1"
                >
                  <template v-slot:button>{{ startHand }}</template>
                </Button>
              </div>
            </div>
            <div v-if="startHand == 'custom'">
              <MultiSelect
                ...
                :label-border="true"
                :listItemClass="
                  (item) => findCardByName(item)?.type || 'orange text-dark'
                "
            ...
            <div v-if="log.length" class="mt-3">
              ...
              <div
                v-for="(action, index) of log"
                :key="action + index"
                class="orange text-dark round mt-1"
              >
                {{ actionToText(action) }}
              </div>
            </div>
...
export default defineComponent({
  ...
  methods: {
    ...
    sortDeck() {
      this.allCards
        .sort((a, b) =>
          findCard(a)
            ?.name.toLowerCase()
            .localeCompare(findCard(b)?.name.toLowerCase())
        )
        .sort(function (a, b) {
          let map = {
            monster: 1,
    ...
    selectSlot(slot: type.Slot) {
      if (this.selectedCard) {
        switch (slot.name) {
          case "enemy deck":
            if (this.selectedCard)
              this.log.push(
                `${this.selectedCard} from ${this.selectedFrom} to deck`
              );
          ...
          case "enemy graveyard":
            if (this.selectedCard)
              this.log.push(
                `${this.selectedCard} from ${this.selectedFrom} to graveyard`
              );
          ...
          case "enemy banish":
            if (this.selectedCard)
              this.log.push(
                `${this.selectedCard} from ${this.selectedFrom} to banish`
              );
          ...
          case "enemy extradeck":
            if (this.selectedCard)
              this.log.push(
                `${this.selectedCard} from ${this.selectedFrom} to extradeck`
              );
            this.addCardToArray("extradeck");
            return;
        }
      }
      if (this.selectedCard && slot.name != this.selectedFrom)
        this.log.push(
          `${this.selectedCard} from ${this.selectedFrom} to ${slot.name}`
        );
      ...
      if (this.field[slotIndex].value[0] != this.field[slotIndex].name) {
        ...
        if (findCard(this.selectedCard)!.type == "xyz") {
          this.field[slotIndex].value.unshift(
            findCard(this.selectedCard)!.name
          );
        } else {
      ...
      switch (this.selectedFrom) {
        ...
        case "extradeck":
          this.allExtraCards.splice(
            this.allExtraCards.indexOf(this.selectedCard),
            1
          );
          break;
        case "banish":
        ...
        default:
          if (fieldSlot!.value.length == 1) {
            ...
            fieldSlot!.value.splice(
              fieldSlot!.value.indexOf(this.selectedCard),
              1
            );
          }
          if (fieldSlot.value.length == 1) {
    ...
    draw() {
      let index = getRandomInt(this.allCards.length);
      this.log.push(`draw ${this.allCards[index]}`);
    ...
    saveLog() {
      if (this.deck.logs) {
        this.deck.logs.push({
          name: this.logName,
          log: this.log,
          startHand: this.logStartHand,
        });
      } else {
        this.deck.logs = [
          { name: this.logName, log: this.log, startHand: this.logStartHand },
        ];
      }
      safeDeck(this.deck);
      this.reset();
    ...
    previousStep() {
      ...
      this.log.pop();
      if (step.includes("draw")) {
        this.handCards.splice(
          this.handCards.findIndex((e) => e == step[1]),
          1
        );
        this.allCards.push(step[1]);
      }
      ...
      switch (step[2]) {
        ...
        default:
          if (this.field[slotIndex].value[0] != this.field[slotIndex].name) {
            ...
            this.field[
              this.field.findIndex((e) => e.name == step[2])
            ].value[0] = card.name;
          }
          break;
      }
