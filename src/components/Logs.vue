    ...
    <div class="modal-content">
      <div class="container">
        ...
        <div v-if="logDeleteCheck" class="mt-3">
          <div class="w-100 orange text-dark round round mb-1">
            Are you sure to delete {{ selectedLog.name }}
          </div>
...
export default defineComponent({
  ...
  methods: {
    ...
    deleteLog(fun: string) {
      if (fun == "delete") {
        this.closeLogModal();
        this.deck.logs = this.deck.logs.filter(
          (e) => e.name != this.selectedLog.name
        );
        this.selectedLog = {};
        this.logDeleteCheck = "";
        safeDeck(this.deck);
    ...
    nextStep() {
      ...
      if (step.includes("draw")) {
        this.allCards.splice(
          this.allCards.findIndex((e) => e == step[1]),
          1
        );
        this.handCards.push(step[1]);
      }
