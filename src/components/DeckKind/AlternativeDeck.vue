<template>
  ...
  <div class="container mt-2">
    ...
    <div v-else>
      ...
      <div
        class="cardTable p-2"
        v-for="card in deck.alternativeCards?.concat(
  ...
  methods: {
    ...
    openCardDeleteModal(card: type.Card | type.ExtraCard) {
      ...
      if (
        ...
        this.deleteCardId = this.deck.alternativeCards.findIndex(
          (c) => c == card
        );
      } else {
    ...
    addCard() {
      ...
      if (
        ...
        if (this.deck.alternativeCards) {
          if (
            this.deck.alternativeCards.findIndex(
              (c) => c.name == this.nameInput
            ) != -1 ||
            this.deck.cards.findIndex((c) => c.name == this.nameInput) != -1 ||
            this.deck.sideCards.findIndex((c) => c.name == this.nameInput) !=
              -1 ||
            this.deck.extraCards.findIndex((c) => c.name == this.nameInput) !=
              -1 ||
            this.deck.alternativeExtraCards.findIndex(
              (c) => c.name == this.nameInput
            ) != -1
          ) {
            this.closeCardAddEditModal();
          ...
          this.deck.alternativeCards.push({
            ...
            value: this.value,
            id: db.find((e) => e.name == this.nameInput)?.id,
          });
        }
      } else {
          ...
          this.alternativeExtraCards = [
            {
              ...
              type: this.type,
              count: parseInt(this.countInput),
              id: db.find((e) => e.name == this.nameInput)?.src,
            },
          ];
        }
      }
    ...
    uniqueCardDeck() {
      this.allCards = [];
      for (let card of this.deck.alternativeCards.concat(
        this.deck.alternativeExtraCards
      )) {
      ...
      this.uniqueAllCards = [
        ...new Set(this.deck.alternativeCards.filter((c) => c.name)),
      ];
    },
    sortDeck() {
      this.allCards
