import ShowPropertiesVue from "./ShowProperties.vue";
<template>
  <link
...
export default defineComponent({
  ...
  methods: {
    ...
    openCardDeleteModal(card: type.Card) {
      ...
      this.deleteCardId = this.deck.cards.findIndex((c) => c.id == card.id);
    ...
    openCardEditModal(card: type.Card) {
      ...
      this.editCardIndex = this.deck.cards.findIndex((c) => c.id == card.id);
      this.primitives.error = {};
      this.editAdd = "edit";
      this.primitives.nameInput = card.name;
      ...
      if (this.deck.cards?.length > 20) {
        if (
          this.deck.cards.map((c) => c.count).reduce((a, b) => a + b) +
            parseInt(this.primitives.countInput) >
          60
        ) {
