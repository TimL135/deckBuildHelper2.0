  ...
  <div class="container mt-2">
    ...
    <div class="d-flex mb-1">
      ...
      <div
        class="w-25"
        style="border: 1px solid #ffa107"
        @click="filter[5] = !filter[5]"
        :class="filter[5] ? 'orange text-dark' : ''"
      >
        Searchable:
        <br />
      ...
      <div
        class="w-25"
        style="border: 1px solid #ffa107"
        @click="filter[6] = !filter[6]"
        :class="filter[6] ? 'orange text-dark' : ''"
      >
        Combo Piece:
        <br />
      ...
      >
        ...
        <div>
          {{
            ...
            (
              (1 - (deckNumber - (card.count + 5)) / (deckNumber - 5)) *
              100
            ).toFixed(2)
          }}%)
        </div>
    ...
    <div class="modal-content">
      <div class="container">
        ...
        <div class="selectModal">
          <div>
            <Button
              @click="openCardDeleteModal(selectedCard?.name)"
          ...
          <div>
            <Button
              @click="openCardEditModal(selectedCard?.id)"
              class="orange round border-0"
            >
              <template v-slot:button>
                <svg
    ...
    <div class="modal-content">
      <div class="container">
        <div class="d-flex justify-content: center mb-1">
          <div class="w-100 orange text-dark round round">
...
export default defineComponent({
  ...
  methods: {
    ...
    openCardEditModal(id: string) {
      ...
      this.countInput = this.deck.cards[this.editCardIndex].count;
    ...
    addCard() {
      ...
      if (this.deck.cards?.length > 20) {
        if (
          this.deck.cards.map((c) => c.count).reduce((a, b) => a + b) +
      ...
      if (this.deck.cards) {
        if (this.deck.cards.findIndex((c) => c.name == this.nameInput) == -1) {
          this.deck.cards.push({
            ...
            value: this.value,
            id:
              this.deck.alternativeCards.find((c) => c.name == this.nameInput)
                ?.id || Math.random().toString().slice(-15),
          });
          this.deck.alternativeCards = this.deck.alternativeCards.filter(
            (card) => card.name != this.nameInput
    ...
    editCard() {
      if (
        this.deck.cards.map((c) => c.count).reduce((a, b) => a + b) -
          this.deck.cards[this.editCardIndex].count +
          parseInt(this.countInput) >
        60
      )
    ...
    checkComboCardGroups() {
      ...
      for (let combo of this.deck.combos) {
        combo.active = combo.cards.every((card) =>
          typeof card === "object"
            ? this.deck.cardGroups.find((c) => c.id == card.id)?.active
    ...
    deckRatingValue() {
      if (this.allCards.length) {
        this.deckValue =
          Math.round(
            (this.allCards.map((c) => c.value).reduce((a, b) => a + b) /
