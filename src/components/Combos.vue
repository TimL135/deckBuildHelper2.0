<template>
  <div class="container" style="margin-top: 3vh">
    ...
    <table class="table table-striped">
      ...
      <tbody>
        <tr
          v-for="combo in deck.combos.filter((c) => c.active)"
                ...
                :listItemClass="
                  (item) =>
                    findCardGroupByName(item.name)
                      ? 'orange text-dark'
                      : findCardByName(item.name)?.type + ' text-dark' ||
                        'orange text-dark'
                "
              />
            </div>
  ...
  methods: {
    ...
    openComboEditModal(index: number) {
      ...
      for (let card of this.deck.combos[index].cards) {
        this.comboCards[
          this.deck.combos[index].cards.findIndex((c) => c == card)
        ] = this.findCard(card)?.name || this.findCardGroup(card)?.name;
      }
      this.changeType();
      var modal = document.getElementById("comboAddEditModal");
      if (modal) modal.style.display = "block";
