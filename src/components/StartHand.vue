<template>
  ...
  <div class="container" style="margin-top: 3vh">
    ...
    <div class="d-flex mb-1">
      <div class="w-100 round" style="border: 1px solid #ffa107">
        Value:{{ value }}
      </div>
    </div>
    <div class="d-flex mb-1">
  ...
  <div id="simulationModal" class="modal">
    <div class="modal-content">
      <div class="container">
        <div
          v-for="result of Object.entries(results)"
          :key="JSON.stringify(result)"
          class="orange round mb-1 text-dark"
        >
  ...
  methods: {
    randomStartHand() {
      ...
      for (let i = 0; i < 5; i++) {
        ...
        this.handCards[i] = findCard(
          this.allCards.splice(index, 1).toString()
        )?.name;
      }
      this.countCard();
    },
              ...
              ? findCardGroup(name).cards.some((cardInCardGroup) =>
                  comboHandCards.includes(findCard(cardInCardGroup)?.name)
                    ? comboHandCards.splice(
                        comboHandCards.findIndex(
                          (card) => findCard(card)?.id == cardInCardGroup
                        ),
                        1
                      )
                    : null
    ...
    openSimulationModal() {
      window.onclick = (event) => {
        if (event.target == document.getElementById("simulationModal"))
    ...
    simulation() {
      ...
      for (let i = 0; i < 100; i++) {
        ...
        if (this.results[this.possibleCombos.length])
          this.results[this.possibleCombos.length]++;
